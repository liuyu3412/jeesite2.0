...
function initSelect() {
    ...
    if (parentid) {
        ...
        if (parentVal == 1) {
            ...
            str += "											data-options=\"validType:'floatNumToNum[2,4]'\"></td>";
            str += "									</tr>";
            str += "                                    <tr><td style=\"text-align: right;\"><a style=\"color: red\">*</a>是否分布式:</td><td><select remark=\"30\" name=\"isbidtype\" class=\"paramSelect required\"></select></td></tr>"
            str += "								</tbody>";
            str += "							</table>";
        } else if (parentVal == 0) {
...
$(function () {
    ...
    function commit(){
        if ($('#baseInfoform').valid()) {
          ...
          /*  var url = "/dljyzx/flow/choosePerson";
            ...
            $('input[data-action="commit"]').css('background-color','#999999');
            $('input[data-action="commit"]').attr('disabled',"true");
            $.ajax({
    ...
    $('input[data-action="commitNoFlow"]').on('click', function () {
    	if ($('#baseInfoform').valid()) {
            ...
            var formData = new FormData($("#baseInfoform")[0])
            $.ajax({
                url: top.ctx + "/dljyzx/baRegGenerator/submitForReviews?checkcode="+checkCode,
                type: "POST",
                data: formData,
                async: false,
                cache: false,
    ...
    $('input[data-action="commitFlow"]').on('click', function () {
        if ($('#baseInfoform').valid()) {
            //判断是否有核准 文件
            var guid = $("#generatorid").val();
            var typeFlag=false;
            ...
            var formData = new FormData($("#baseInfoform")[0])
            $.ajax({
                url: top.ctx + "/dljyzx/baRegSeller/submitForReview?checkcode="+checkCode,
                type: "POST",
                data: formData,
                async: false,
                cache: false,
                ...
                success: function (data) {
                    if (data) {
                        var generatorid = data.generatorid;
                        if (data!=null) {
                            $('input[data-action="commitFlow"]').attr('disabled',"true");
                            top.$.jBox.alert("新增成功!");
                            window.location.href= top.ctx+"/dljyzx/baRegSeller/baRegGeneratorInfosRegist?type=regist&participantid="+getQueryString("participantid")+"&jobnum="+getQueryString("jobnum");
                        }
                    }
                }
            })
...
function validateFrom(){
    ...
    var generatortype=$("#generatortype option:selected").val();
        //判断是否上传建设核准文件
        //判断是否有核准 文件
        var guid = $("#generatorid").val();
        var typeFlag = false;
    ...
    if(generatorname == "" || generatortype=="" || gateidId=="" || opbegindate=="" ||  dcjzId==""
