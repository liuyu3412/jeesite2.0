...
function getClickPage(thepage,pageSize,val){
	var divs = document.getElementsByClassName("tab_css");  
	//var choosePage = document.getElementById("tabPage").value;
    for(var i = 0; i < divs.length; i++) {  
       var divv = divs[i];  
...
function gePrecision() {
	...
	var difference = null;
	var powerEnergyType = null;
	var vendeeispricedifference = null;
	var isdeconeenergy=null;//售方申报一个电量
	var vendeedecnum =1;//购方申报时选择售方数量 默认为1
	var istradeyeartomonths = null;
	var powerPrecision = null;
	var sumCapacity = null;
	var issalemarginalprice = null;
		...
		if(precision.list.length>0){
			...
			for(var j = 0; j < precisionNum.length; j++){
				if("ENGERY_PRECISION"==precisionNum[j][0]){
					...
					vendeeispricedifference = precisionNum[j][0];
	...
	if(pricePrecision!=null){
		if(pricePrecision > 0){
			tex += "按照"+Math.pow(10,pricePrecision)+"元/兆瓦时的整数倍申报电价；电量单位："+unit+"，容量单位："+rldw+"，电价单位：元/兆瓦时";
		}else{
			tex += "申报电价精确到"+Math.pow(10,pricePrecision)+"元/兆瓦时；电量单位："+unit+"，容量单位："+rldw+"，电价单位：元/兆瓦时";
		}
	}else{
		tex += "申报电价精确到0.01元/兆瓦时；电量单位："+unit+"，容量单位："+rldw+"，电价单位：元/兆瓦时";
	} 
	
	$("#reportTimelabel")[0].innerHTML =tex;//给label赋值;
	
	if (difference == null || difference == "") {
		if(vendeeispricedifference=="1" ){
...
function checkEngPrice(){
	...
	if(bandNum != "1"){
		...
		for(var i = 0; i < etableRow[0].rows.length; i++){
			...
			//if(erowCheck.checked){
				...
				if(( i % bandNum ) == 0){
					bandGroup[num++] = {"egroupId" : jydyIds, "totalEng" : evryEng, "egroupEngHi" : energyhi, "egroupEnglow" : energylow };
				}else{
		...
		for(var n = 0; n < bandGroup.length; n++){
			...
			if(eGroupEngHi != null && eGroupEngHi != ""){
				if(eGroupEng > eGroupEngHi){
					return [false,"相同交易单元、时间段，申报电量（总）必须小于电量（总）上限" + eGroupEngHi +"兆瓦时！",(n * bandNum),"energy"];
				}
			}
			if(eGroupEnglow != null && eGroupEnglow != ""){
				if(eGroupEng < eGroupEnglow){
					return [false,"相同交易单元、时间段，申报电量（总）必须大于电量（总）下限" + eGroupEnglow +"兆瓦时！！",(n * bandNum) ,"energy"];
				}
			}
		}
		...
		for(var a = 0; a < etableRow[0].rows.length; a++){
			...
			if(erowCheck.checked){
				epriceGroup[a] = {"jydyid" : jydyId, "price" : declarPrice};
				for(var b = 0; b < etableRow[0].rows.length; b++){
					...
					if(b!=a){
						if(epriceGroup[a].jydyid == nextjydy && epriceGroup[a].price == nextprice
	...
	for(var i = 0; i < tableRow[0].rows.length; i++){
		...
		//if(rowCheck.checked){ 
			...
			if(count == 0){
				groupIdArr[count++] = {"groupId" : groupId, "totalEng" : 0, "groupEngHi" : groupEngHi, "groupEnglow" : groupEnglow };
			}else{
	...
	for(var j = 0; j < tableRow[0].rows.length; j++){ 
		...
		var userMaxPower = tableRow.find("input[id='userMaxPower']")[j]== null? null : tableRow.find("input[id='userMaxPower']")[j];
		if(rowCheck.checked){
			...
			if(pricePrecision == null){
				...
				if(!bool2){
					if(rowPrice){//电价的验证 保留1位小数
						return [false,"请输入最多6位整数,最多" + Math.abs(pricePrecision) + "位小数的数字！",j,"price"];
						break ;
					}
				}
			}else{
				...
				if(!bool2){
					if(rowPrice){//电价的验证 保留1位小数
						var tex = "";
						if(pricePrecision >= 0){
							tex +=  "电价必须为" + Math.pow(10,pricePrecision) + "的整数倍！";
						}else{
							tex += "请输入最多6位整的数,最多" + Math.abs(pricePrecision) + "位小数的数字！"
						}
						if(tex != null && tex != ""){
			...
			if(energyPrecision != null){
				...
				if(!bool3 && Number(energy) != 0){
					if(energy){//电量的验证 10的整数倍
						var tex = "";
						if(energyPrecision < 0){
							tex += "请输入最多10位整数,最多" + Math.abs(energyPrecision) + "位小数的数字！"
						}else{
							if(parseFloat(rowEng.value) % Math.pow(10,energyPrecision) != 0){
								tex += "电量必须为" + Math.pow(10,energyPrecision) + "的整数倍！";
							}
						}
						if(tex != null && tex != ""){
			...
			if(bandNum == "1"){
				...
				var hi_lmt = tableRow.find("input[id='energyhi']")[j].value == ""?null:Number(tableRow.find("input[id='energyhi']")[j].value);
				var low_lmt = tableRow.find("input[id='energylow']")[j].value == ""?null:Number(tableRow.find("input[id='energylow']")[j].value);
				var showText = "";
				if(energy && (hi_lmt ||low_lmt)){
					if(hi_lmt == null){
						if(low_lmt != null){
							if(energy < low_lmt){
								showText = "相同交易单元、时间段，申报电量（总）必须大于电量（总）下限"+energy_low_lmt_checked+"兆瓦时";
							}
						}
					}else{
						if(low_lmt != null){
							if(energy < low_lmt || energy > hi_lmt){
								showText = "相同交易单元、时间段，申报电量（总）必须在电量（总）上限"+hi_lmt+"兆瓦时和下限"+low_lmt+"兆瓦时之间";
							}
						}else if(energy > hi_lmt){
							showText = "相同交易单元、时间段，申报电量（总）必须小于电量（总）上限"+hi_lmt+"兆瓦时";
						}
					}
				}
			...
			var hi_price = tableRow.find("input[id='pricehi']")[j].value==""?null:Number(tableRow.find("input[id='pricehi']")[j].value);
			var low_price = tableRow.find("input[id='pricelow']")[j].value==""?null:Number(tableRow.find("input[id='pricelow']")[j].value);
			var rulePriceHi = tableRow.find("input[id='rulePriceHi']")[j].value == "" ? null : Number(tableRow.find("input[id='rulePriceHi']")[j].value);
			var rulePricelow = tableRow.find("input[id='rulePricelow']")[j].value =="" ? null : Number(tableRow.find("input[id='rulePricelow']")[j].value);
			var priceText ="";
			if(price !=null && (hi_price ==null || low_price ==null)){
				if(rulePriceHi == null){
					if(rulePricelow != null){
						if(price < rulePricelow){
							priceText = "申报价格不能低于价格下限" + rulePricelow + "元/兆瓦时！";
						}
					}
				}else {
					if(rulePricelow != null){
						if(price < rulePricelow || price > rulePriceHi){
							priceText = "申报电价必须在电价上限" + rulePriceHi + "元/兆瓦时和下限" + rulePricelow + "元/兆瓦时之间！";
						}
                    }else {
                        if(price > rulePriceHi){
                            priceText = "申报价格不能高于价格上限" + rulePriceHi + "元/兆瓦时！";
                        }
					}
				}
			}else if(price && (hi_price || low_price)){
				if(hi_price == null){
					if(low_price != null){
						if(price < low_price){
							priceText = "申报价格不能低于" + low_price + "售方申报价格下限！";
						}
					}
				}else{
					if(low_price != null){
						if(price < low_price || price > hi_price){
							priceText = "申报电价必须在电价上限" + hi_price + "元/兆瓦时和下限" + low_price + "元/兆瓦时之间！";
						}
		...
		var evryEng = tableRow.find("input[id='declarEng']")[j].value != null?Number(tableRow.find("input[id='declarEng']")[j].value) : 0 ;
	...
	for(var n = 0; n < groupIdArr.length; n++){
		...
		if(evryGroupEngHi != null && evryGroupEngHi != ""){
			if(evryGroupEng > evryGroupEngHi){
				return [false,"申报的电量总和不能大于单元组电量限额总上限！",0,null];
			}
		}
		if(evryGroupEnglow != null && evryGroupEnglow != ""){
			if(evryGroupEng < evryGroupEnglow){
				return [false,"申报的电量总和不能小于单元组电量限额总下限！",0,null];
			}
		}
	}
}
...
{   
	...
	var checkcode = $("#checkcode")[0].value;
	var tableRows = $("#contentTable").find('tbody');
	//判断是否有选择
	...
	var index = 0;
	//var tableRows = $("#contentTable").find('tbody');
	for(var i = 0; i < tableRows[0].rows.length; i++){
		var rowCheck = tableRows.find("input[id='checkId']")[i];
		if(rowCheck.checked){
			var seqId = rowCheck.value; 
			var rowTradeCode = tableRows.find("input[id='tradetypeCode']")[i].value;
			var rowBidinfoId = tableRows.find("input[id='bidinfoid']")[i].value;
			var rowEng = tableRows.find("input[id='declarEng']")[i].value;
			var rowPrice = tableRows.find("input[id='declarPrice']")[i].value;
			var userMaxPower = rowTradeCode.substring(0,3)== '200' ? tableRows.find("input[id='userMaxPower']")[i].value : "";
			var bandid = tableRows.find("input[id='bandid']")[i].value;
            var jydyIds = tableRows.find("input[id='jydyIds']")[i].value;
			var param = {
				"seqId" : seqId,
				...
				"rowEng" : rowEng,
				"rowPrice" : rowPrice,
				"userMaxPower" : userMaxPower,
                "bandid" : bandid,
                "jydyIds" : jydyIds
			};
			
			//JsonStr += '{\"seqId\":' + seqId + ',\"rowEng\":' + rowEng + ',\"rowPrice\":' + rowPrice + ',\"userMaxPower\":' + userMaxPower + ',\"rowTradeCode\":' + rowTradeCode + ',\"rowBidinfoId\":\"' + rowBidinfoId +'\"}';
			tableRow[index++] = param;
		}
		//alert(tableRow[i-1].seqId + " " + tableRow[i-1].rowEng + " " +  tableRow[i-1].rowPrice + " " +  tableRow[i-1].rowTradeCode);
	}
	...
	if(res != undefined && !res[0]){
		if(res[1]){
			if(res[3] != null && res[3] == "energy"){
				...
				$('#contentTable').find('tbody').find("input[id='declarPrice']")[res[2]].style.borderColor = "";
			}else if(res[3] != null && res[3] == "price"){
				$('#contentTable').find('tbody').find("input[id='declarPrice']")[res[2]].style.borderColor = "red";
				$('#contentTable').find('tbody').find("input[id='declarEng']")[res[2]].style.borderColor = "";
			}else if(res[3] != null && res[3] == "power"){
				$('#contentTable').find('tbody').find("input[id='userMaxPower']")[res[2]].style.borderColor = "red";
			} 
			jBox.alert(res[1],"消息");
		}
		return ;
	...
	//var dataEncryptFlag = isEncrypt.DATA_ENCRYPT_FLAG;
	var bandNum = $("#bundNum").val();
	...
	if (tableRow.length > 0) {
		...
		for(var k= 0; k < tableRow.length; k++){
			...
			var price;
			var userMaxPower;
			var bandid = tableRow[k].bandid.toString();
			...
			temp1 += "<span style='font-size:17px;padding-left:10px;'>"
					+ ("第" + bandid + "段  电量：" + tableRow[k].rowEng + "MWh="
							+ Math.round(tableRow[k].rowEng) / 10 + "万千瓦时="
							+ Math.round(tableRow[k].rowEng) / 100000 + "亿千瓦时 " 
							+ ((bandNum != "1")? "，占总量比：" + ((Number(evryGroupEng) == 0) ? "--" : Math.round(tableRow[k].rowEng / evryGroupEng * 100) + "%") : "")
							+ "&nbsp;\；&nbsp;电价："
			...
			if(dataEncryptFlag == 1){ //如果需要加密，则对数据进行加密处理
				...
				userMaxPower =  tableRow[k].userMaxPower ;
			}else{
				bidinfoid = tableRow[k].rowBidinfoId;
			...
			if (typeCode.substr(0,3)=='100') {
				array[k] = bidinfoid + ";" + jydyId + ";" + tradeRole + ";"
						...
						+ energy + ";" + price + ";" 
						+ energyPeriod1 + ";" + pricePeriod1 + ";" + energyPeriod2 + ";"
				...
				//加密  array[k] = bidinfoid+";"+jydyId+";"+tradeRole+";"+band+";"+tradeTimepart+";"+timeRange+";"+getEncryptMessage(energy)+";"+getEncryptMessage(price);
				var val = {
					"bidinfoid" : bidinfoid,
					"energy" : energy,
					"price" : price,
				...
				array[k] = val;
				//array[k] = '{\"bidinfoid\":' + bidinfoid + ',\"energy\":' + energy + ',\"price\":' + price + ',\"userMaxPower\":' + userMaxPower +'}';
		...
		top.jBox(temp1 + "</div>", {
			...
			submit : function(v, h, f) {
				if (v == 'ok') {
			    	...
			    	jsonData['dataEncryptFlag'] =  dataEncryptFlag;// 增加加密标志
			    	jsonData['dataDisgerFlag'] = dataDisgerFlag;//dataDisgerFlag;// 增加加密标志
			    	
			    	var result = commenAjax("saveData?checkcode=" + checkcode,{"params":JSON.stringify(jsonData)});
			    	if(result.success){
			    		top.$.jBox.alert("申报成功！","消息");
			    	}else{
			    		top.$.jBox.alert("申报失败！","消息");
			    	}
			    	$("#searchForm").submit();
			    }else{
					top.jBox.close();
...
function getCheckedList(){
	...
	var checkde_val = [];
	for(var i = 0; i < tableRows[0].rows.length; i++){
		var erowCheck = tableRows.find("input[id='checkId']")[i];
		if(erowCheck.checked){
...
function getGlobalValue(){
	...
	if(dataEncryptFlag != null && dataEncryptFlag == '1'){ //等于1时才需要加密， 
		...
		if(encryptInfo) {
			...
			if(dataEncryptFlag == '1' && !certContent) {//启用加密没有获取证书时需要给出提示，改为非加密数据
				top.$.jBox.alert("获取加密证书失败，请联系管理员进行确认！继续申报将存在安全隐患！","提示");
				dataEncryptFlag = '0';  
			}
		}
	}
	...
	if(dataEncryptFlag == '1') {
		try{
			...
			var road = this.ctx.toString().substring(0,this.ctx.toString().indexOf("admin"));
			var chajian = "<a target='_blank' style='text-decoration:underline;color:red' href='" + road + "plugs/npCryptoKit.Standard.x86.exe'>加密插件</a>】，如已下载，请忽略！";
			if(window.parent.window.myflag == "true"){
				top.$.jBox.alert("未获取到加密插件，继续申报将存在安全隐患，请下载公共基础插件【" + chajian +"再使用360浏览器（极速模式）申报！","提示");
			}
			dataEncryptFlag = '0';
			dataDisgerFlag = '0';
...
function commenAjax(methodName,params){  
	var items = null;
	$.ajax({
		url : this.ctx + "/dljyzx/jsTradeengup/" + methodName,
		dataType : "json",
		data : params,
		type : 'post',
		async: false,
