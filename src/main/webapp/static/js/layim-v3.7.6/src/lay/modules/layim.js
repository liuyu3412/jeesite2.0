...
//查询某条数据相关信息
var selectLayui = function (id, log, callback) {
...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    var elemTpl = ['<div class="layui-layim-main">'
        ...
        // , '<li {{d.mine.status === "hide" ? "class=layim-this" : ""}} layim-event="status" lay-type="hide"><i class="layui-icon">&#xe605;</i><cite class="layui-icon layim-status-hide">&#xe60f;</cite>离线</li>'
        // , '</ul>'
        // , '</div>'
        ...
        , '{{# } }}'
        , '{{# if(d.base.groupfind){ }}'
        , '<li class="layui-icon layim-tool-groupfind" layim-event="groupfind" title="创建群">&#xe770;</li>'
        , '{{# } }}'
    ...
    var setHistory = function (data) {
        ...
        var historyCache = layui.data("history");

        if (data.id.indexOf('GROUP') == -1) {
            if (historyCache[data.id] == 'online') {
                data.status = 'online';
            } else {
                data.status = 'offline';
            }
    ...
    var anim = 'layui-anim-upbit', events = {
                    ...
                    , success: function (layero) {
                        ...
                        post(members, function (res) {
                            layui.each(res.list, function (index, item) {
                                li += '<li data-uid="' + item.id + '"><a href="javascript:;"><img src="' + item.avatar + '"><cite title="'+item.username+'">' + item.username + '</cite></a></li>';
                            ...
                            ul.find('li').on('click', function () {
                                ...
                                var mine = cache.mine; //包含我发送的消息及我的信息
                                var groupId = ($('.layim-chat-other')[0].children[0].className).replace('layim-group', '');
                                selectLayui(groupId, 'groupsLog', function (res) {
                                        if (res) {
                                            if (res.sysAdminId == mine.id) {
                                                if (mine.id != uid) {
                                                    parent.layer.open({
                                                        type: 1
                                                        ,
                                                        ...
                                                        ,
                                                        content: '<div style="padding: 50px; line-height: 0px; background-color: #393D49; color: #fff; font-weight: 300;">是否确认踢出该群员？</div>'
                                                        ,yes: function (group, index) {
                                                            //实际部署时，请开启下述注释，并改成你的接口地址
                                                            $.post($('#ctx').val() + '/dljyzx/im/delGroupFriend', {
                                                                receiverId: uid //对方用户ID
                                                                , senderid: mine.id
                                                                , type: 1
                                                                , text: ''
                                                                , name: ''
                                                ...
                                                if (mine.id == uid) {
                                                    parent.layer.open({
                                                        ...
                                                        moveType: 1 //拖拽模式，0或者1
                                                        ,
                                                        content: '<div style="padding: 50px; line-height: 0px; background-color: #393D49; color: #fff; font-weight: 300;">是否确认退出该群？</div>'
                                                        ,yes: function (group, index) {
                                                            //实际部署时，请开启下述注释，并改成你的接口地址
                                                            $.post($('#ctx').val() + '/dljyzx/im/delGroupFriend', {
                                                                receiverId: uid //对方用户ID
        ...
        , menuHistory: function (othis, e) {
            var local = layui.data('layim')[cache.mine.id] || {};
            var othisParent = othis.parent(), type = othis.data('type');
            var hisElem = layimMain.find('.layim-list-history');
            var none = '<li class="layim-null">暂无历史会话</li>'

            if (type === 'one') {
                var history = local.history;
                delete history[othisParent.data('index')];
                local.history = history;
                layui.data('layim', {
                    key: cache.mine.id
                    , value: local
                });
                $('#' + othisParent.data('id')).remove();
                $(".layim-list-history li").each(function () {
                    if ($(this).attr('data-index') == othisParent.data('index')) {
                        $(this).remove();
                    }
                });
