...
function init(){
	...
	if($('#message').val() == 0){
		...
		ysorNo = false;
		if(ddataStatus == 10 || ddataStatus == 12 || ddataStatus == 20 || ddataStatus == 22 || ddataStatus == 30 || ddataStatus == 32){
			id31=true;
			id30=true;
			id219=true;
			id44=true;
			...
			$('#back').attr('flag','0');
			$('#back').attr('class','btn-primary');
		}else if(ddataStatus == '01'){
			if($('#regMarjet').val() == 0){
				...
				$('#sa4').css('display','block');
                $('.addFile').css('display','none');
				if($('#waitDo').val() == 1){
					$('#sa6').css('display','block');
				}
	...
	if(ysorNo == true){
		...
		if($('#depositname').val() == ''){
			...
			if (re.test(depositname)) {
				...
				jBox.tip("请输入正确的中文-账户名称！", "messager");
				$('#depositname').css({'border-color':'rgba(245,88,88,0.3)','box-shadow':'0 0 7px rgba(245,88,88,2)'}); 
				$('#depositname').attr('class','false');
				return false;
			}
				 ...
				 var guid=baGuidList[i].guid;
					   if(baGuidList[i].affixtype==31){
						  ...
						  var guid44=guid;
							...
							guidid.find('a').eq(2).attr('style','text-decoration: underline;cursor: pointer;color:#4F81BD;');
							id44=true;
					 }else if(baGuidList[i].affixtype==60){
                           var guid60=guid;
                           var guidid = $(obj).parent();
                           guidid.find('a').eq(1).unbind("click").bind("click",function(){
                               delFile(guid60,obj,60);
                           });
                           guidid.find('a').eq(1).attr('guid',guid60);
                           guidid.find('a').eq(1).attr('affixtype',60);
                           guidid.find('a').eq(2).bind("click",function(){
                               downLoadFile(guid60,obj);
                           });
                           guidid.find('a').eq(0).attr('style','display: none;text-decoration: underline;cursor: pointer;color:#4F81BD;');
                           guidid.find('a').eq(1).attr('style','text-decoration: underline;cursor: pointer;color:#4F81BD;');
                           guidid.find('a').eq(2).attr('style','text-decoration: underline;cursor: pointer;color:#4F81BD;');
...
function initFileGrid2(consid,obj) {
    //加载附件地址
    var url = $('#ctx').val()+"/PowerConsumerView/bindview/getFileList";
    $.ajax({
        url :url,
        ...
        success : function(data){
            ...
            for(var i=0;i<baGuidList.length;i++){
                ...
                if(baGuidList[i].affixtype==31){
                    ...
                    guidid.find('a').eq(0).attr('style','display: block;text-decoration: underline;cursor: pointer;color:#4F81BD;');
                    guidid.find('a').eq(1).attr('style','text-decoration: underline;cursor: pointer;color:#4F81BD;');
                    guidid.find('a').eq(2).attr('style','text-decoration: underline;cursor: pointer;color:#4F81BD;');
                    id30=true;
                }else if(baGuidList[i].affixtype==29){
...
function addFiles(obj){
	...
	var consid = $('#considq',parent.document).val();
	...
	top.$.jBox('iframe:'+$('#ctx').val()+'/PowerConsumerView/bindview/UserFileUp', {
		...
		ajaxData : {sign:sign,consid:consid,dataStatus:ddataStatus,allStatus:false}, /*
    ...
    top.$.jBox('iframe:'+$('#ctx').val()+'/PowerConsumerView/pcview/fileUp?start=1', {
        ...
        closed: function () {  /* 窗口关闭后执行的函数 */
            if(window.parent.parent.window.isFreshFlag=='2'){
                ...
                initFileGrid2(window.parent.parent.window.consid,obj);
            }
        }
    });
}
...
function addFiles(status,obj){
	...
	top.$.jBox('iframe:'+$('#ctx').val()+'/PowerConsumerView/bindview/UserFileUp', {
		...
		persistent : true, /* 在显示隔离层的情况下，点击隔离层时，是否坚持窗口不关闭 */
		showScrolling : false, /* 是否显示浏览的滚动条 */
		ajaxData : {sign:sign,status:status,consid:consid,sessionid:sessionid,dataStatus:ddataStatus,allStatus:false}, /*
						 * 在窗口内容使用post:前缀标识的情况下，ajax post的数据，例如：{ id: 1 } 或
						 * "id=1"
						 */
		iframeScrolling : 'yes', /* 在窗口内容使用iframe:前缀标识的情况下，iframe的scrolling属性值，可选值有：'auto'、'yes'、'no' */
...
function addFiles1(status,obj){
	...
	top.$.jBox('iframe:'+$('#ctx').val()+'/PowerConsumerView/bindview/UserFileUp1', {
		...
		persistent : true, /* 在显示隔离层的情况下，点击隔离层时，是否坚持窗口不关闭 */
		showScrolling : false, /* 是否显示浏览的滚动条 */
		ajaxData : {sign:sign,status:status,consid:consid,sessionid:sessionid,dataStatus:ddataStatus}, /*
						 * 在窗口内容使用post:前缀标识的情况下，ajax post的数据，例如：{ id: 1 } 或
						 * "id=1"
						 */
		iframeScrolling : 'yes', /* 在窗口内容使用iframe:前缀标识的情况下，iframe的scrolling属性值，可选值有：'auto'、'yes'、'no' */
...
//提交审批信息
function svSubmit(){
	_startFlow($('#participantname').val());
}
//查看审批信息
function upSubmit(){
...
function submit(message){
    ...
    if(id60==false){
        jBox.tip("请上传电子邮箱手机号授权文件！", "messager",{ id:null,top:'75%' });
        return false;
    }
	debugger;
	//保存信息路径
	var url =  $('#ctx').val()+'/BaaddSaleCompanyView/bindview/saveSubmit?message='+$('#message').val()+'&ddataStatus='+$('#dataStatus',parent.document).val()+'&checkcode='+$('#checkcode').val();
	...
	$.post(url,date,function(data) {
		if(data.message == 0){
            ...
            $('#e-mail').val($('#email').val());
			if($('#addStatus',parent.document).val() == 0){
				window.setTimeout(function () {
					var div1 = window.parent.document.getElementById("div1");
					var div2 = window.parent.document.getElementById("div2");
					var a = window.parent.document.getElementById("a");
					var b = window.parent.document.getElementById("b");
					div1.style.display = "none";
					div2.style.display = "block";
					a.className = "bl";
					b.className = "current bl";
				},2000);
			}
			if($('#dataStatus',parent.document).val() == '01' && $('#regMarjet').val() == 0){
				_startFlow($('#participantname').val());
				//location.reload();
			}
					...
					$.post(url, {guid : $(this).attr('guid')}, function(data) {
						if(data.sunccess){
							if($(this).attr('affixtype') == 31){
								...
								id219=false;
							}else if($(this).attr('affixtype') == 60){
                                id60=false;
                            }
						}else{
							jBox.tip("删除失败！", "messager",{ id:null,top:'75%' });
