...
function addOtherFile(status) {
    ...
    top.$.jBox('iframe:' + path + '/homePage/UserFileUp', {
        id: null,
...
function businessCodeVali() {
    ...
    if (!saved) {
        $.ajax({
            url: path + "/homePage/checkBusinesscode",
            type: "POST",
            data: {
...
function findGudongPer() {
    //股东信息是否为百分百
    var url = path + "/homePage/findbaGengroup";
    $.ajax({
        url: url,
        type: "post",
        dataType: "json",
...
function savedatefunction() {
    ...
    if ($('#form').valid()) {
        ...
        var url = "addPowerPlantsInfo"
        //获取form中数据
        var formData = new FormData($("#form")[0]);
        formData.append("isNew", "new");
        formData.append("seqId", $('#sid').val());
        $.ajax({
            url: url,
            type: "POST",
            data: formData,
...
function initFileGrid() {
    var downName = "下载查看";
    //加载附件地址
    var url = path + "/homePage/getFileList";
    var participantid = $("#participantid").val();
    var participanttype = $("#participanttype").val();
    //
    $.ajax({
...
function loadFormData() {
    //清空数据
    $.ajax({
        url: path + "/homePage/loadFormData",
        type: "POST",
        async: false,
        dataType: "json",
        data: {
...
function delFile(guid) {
    top.$.jBox.confirm('确定删除该条记录?', '确认窗口', function (isOk) {
        ...
        var url = path + "/homePage/deleteFile";
        $.ajax({
            url: url,
            type: "POST",
            data: {"guid": guid},
...
function downLoadFile(guid) {
    //下载附件地址（公共方法）
    var sourceguid = guid;
    var url = path + "/homePage/downLoadFile?sourceGuid="
        + guid;
    //打开下载窗口
    window.open(url, "_parent");
}
...
function getParticipantCode() {
    $.ajax({
        url: path + "/homePage/getParticipantCode",
        type: "POST",
        async: false,
        dataType: "json",
        data: {},
...
function next() {
    ...
    if (participanttype == "2") {
        top.jBox.confirm('是否光伏企业！', '', function (v, h, f) {
            ...
            //发电企业 注册发电组
            window.location.href = path + admin + "/dljyzx/baRegGenerator/baRegGeneratorInfosRegist?type=regist&participantid=" + participantid + "&participantName=" + participantName;
            return true;
        ...
        //用户 注册用电单元
        localStorage.setItem("dlyhrk", "1")
        window.location.href = path + admin + "/dljyzx/baMktaadmin/toRegist?type=regist&participantid=" + participantid + "&participantname=" + participantName + "&dlyh=1";
    }
    else if (participanttype == "6") {
        ...
        window.location.href = path + "/homePage/registAccountSale?sellType=" + sellType;
    }
...
function peoples() {
    ...
    var url = path + "/homePage/peopleInfoList?participantid="
        + particid;
// 全局弹出
    top.$.jBox("iframe:" + url, {
        title: "从业人员配置信息",
...
function gudong() {
    ...
    var url = path + "/homePage/dgsPage?type=1&participantid="
        + particid;
// 全局弹出
    top.$.jBox("iframe:" + url, {
        title: "股东信息",
...
function energy() {
    var particid = $("#participantid").val();
    var url = path + "/homePage/baRegSellerList?participantid=" + particid;
