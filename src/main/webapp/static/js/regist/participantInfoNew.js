...
function checkField(value) {
    var test=value.trim().replace(/\s/g,"");
    console.log(test)
    $("#depositname").val(test);
    $("#participantname").val(test);
}
           ...
           /* var tradebelong = $("#tradebelong").val();//所属行业
            ...
            $.ajax({
               // url: "addPowerPlantsInfo?checkcode=" + checkCode,
                url: "addPowerPlantsNewInfo?checkcode=" + checkCode,
                type: "POST",
                data: formData,
                async: false,
                cache: false,
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (participanttype == "0" || participanttype == "3") {
                    if (data[i].affixtype == 23) {
                        ...
                        id337 = true;
                    }else if (data[i].affixtype == 60) {
                        var guid329 = guid;
                        $("#id6_0").unbind("click");
                        $("#id6_0").text(" ").removeAttr("onclick").click(
                            function () {
                                showFile(guid329);
                            }
                        );
                        $("#id60").unbind("click").bind("click", function () {
                            delFile(guid329);
                        });
                        $("#ids60").bind("click", function () {
                            downLoadFile(guid329);
                        });
                        document.getElementById("id60").style.display = "";
                        document.getElementById("ids60").style.display = "";
                        id236 = true;
                    }
                    ...
                    if (data[i].affixtype == 1) {
                        ...
                        $("#ids60").text(downName).bind("click", function () {
                            downLoadFile(guid60);
                        });
                        $("#id60").unbind("click").bind("click", function () {
                            delFile(guid60);
                        ...
                        document.getElementById("id60").style.display = "";
                        if (para_val == "95412") {
                            document.getElementById("id6_0").style.display = "none";
                        }
                        id236 = true;
                    } else {
...
function next() {
    ...
    if (participanttype == "2") {
        ...
        $("#resultTable tbody tr").each(function () {
            if ($(this).find("td:eq(5)").html() == "是") {
                telephone=$(this).find("td:eq(2)").html();
            }
        });
        var isauthinfo = $("#isauthinfo").val()
        if (isauthinfo =='是') {
           /* if(!id236){
                top.$.jBox.alert("请先上传授权认证书！", '消息');
                return false;
            }*/
        ...
        $.ajax({
            ...
            success: function (rc) {
                if (rc) {
                    if (rc.flag == "success") {
                        $.ajax({
                            url: "submitPowerPlantsNewInfo?checkcode=" + checkCode,
                            type: "POST",
                            data: {participantid:participantid},
                            success: function (rc) {
                                if (rc) {
                                    if (rc.flag == "success") {
                                        alert("提交成功，审核通过后即可登录！您的账号为："+businesscode+", 默认密码为:"+telephone +" 审核信息请关注注册公开详情栏目");
                                            window.location.replace("lingshouzhuce");
                                    } else {
                                        top.$.jBox.alert("提交信息失败!", "消息");
                                    }
                                }
                            }
                        })
...
function LoadFile() {
    if (participanttype == "0" || participanttype == "3") {
      ...
      /*  $("#id3_1").unbind().bind("click", function () {
        ...
        $("#id6_0").unbind().bind("click", function () {
            addFile(60);
        }).text("上传附件");
     ...
     /*   document.getElementById("ids31").style.display = "none";*/
        document.getElementById("id60").style.display = "none";
        document.getElementById("ids60").style.display = "none";
...
$(function (){
    $('#isauthinfo').change(function(){
      if($(this).children('option:selected').val()=='是'){
           $('#authorizeTypes').show();
           $('#authorizeType').show();
           $('#id6_0').parents("tr").show();
          $('#id6_0').parents("tr").next("tr").show();
      }else{
          $('#authorizeTypes').hide();
          $('#authorizeType').hide();
          $('#id6_0').parents("tr").hide();
          $('#id6_0').parents("tr").next("tr").hide();
      }
    })
})
