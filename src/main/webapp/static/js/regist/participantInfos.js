...
function savedatefunction() {
    ...
    if (pType == "2") {
        ...
        //保存股东信息
        $.ajax({
          /*  url: "baRegGengroup?checkcode=" + checkCode, //老流程保存股东信息*/
            url: "jsRegGengroup?checkcode=" + checkCode,
            type: "post",
            dataType: "json",
            async: false,
            data: {
    ...
    if ($('#form').valid()) {
        ...
        if (pType == "6") {
            ...
            if (pType == "2") {
               var url="addPowerPlantsNewInfo?checkcode="
            }else{
                var url="addPowerPlantsInfo?checkcode="
            }
            //获取form中数据
            var formData = new FormData($("#form")[0]);
            $.ajax({
