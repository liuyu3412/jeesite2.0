...
$(function () {
    ...
    if (flag =='9') {
        ...
        $('.showText').text($('input[name=jremark]').val());
    ...
    $("#addContact").click(function () {
        ...
        isFreshFlag = "1";
        var url = top.ctx + "/dljyzx/basicInfo/powerPlantsInfo/addContactsDetailsInfojsp?isAddPeople="+isAddPeople+"&checkcode="+checkCode+"&jobNum="+jobNum;
        //全局弹出
        top.$.jBox("iframe:" + url, {
            title: "新增联系人",
            width: 600,
        ...
        $(o).on('click', function () {
            ...
            if ($('#searchForm').valid()) {
                ...
                if(roleNum=="1"){
                    ...
                    $("input[data-action=commit]").attr('disabled','disabled');
                    $.ajax({
                        url: top.ctx + "/dljyzx/basicInfo/powerPlantsInfo/jsRegGengroup?checkcode="+checkCode+"&jobNum="+jobNum,
                        type: "post",
                        async: false,
                        data: {
                            json: JSON.stringify(groupRelationList)
                    ...
                    //保存缓存中的 联系人
                    $.ajax({
                        url: top.ctx + "/dljyzx/basicInfo/powerPlantsInfo/jsReglinkman?checkcode="+checkCode+"&jobNum="+jobNum,
                        type: "post",
                        async: false,
                        data: {
                            json: JSON.stringify(groupRelationList)
                ...
                //保存企业信息
                $.ajax({
                    url: ctx + "/dljyzx/flow/saveOrUpdateNew?checkcode="+checkCode,
                    type: "post",
                    data: json,
                    async: false,
                    dataType: "json",
                    success: function (data) {
                        ...
                        top.jBox.alert('保存成功');
                        setTimeout(function () {
                            window.location.href =ctx +  "/dljyzx/basicInfo/powerPlantsInfo/powerPlantInfo";
                        }, 2000);
    ...
    var inttRelation = function () {
        ...
        var jobNum=$('input[name=jobNum]').val();
        $.post(top.ctx + "/dljyzx/basicInfo/powerPlantsInfo/queryRelationLists?jobNum="+jobNum, function (data) {
            var relationHtml = '';
            isAddPeople = true;
            ...
            $('#resultTable').find('a[data-action="edit"]').each(function (i, o) {
                $(o).on('click', function () {
                    ...
                    var url = top.ctx + "/dljyzx/basicInfo/powerPlantsInfo/addContactsDetailsInfojsp?guid=" + guid+"&isAddPeople="+isAddPeople+"&isCommon="+isCommon+"&checkcode="+checkCode+"&jobNum="+jobNum;
                    //全局弹出
                    top.$.jBox("iframe:" + url, {
                        title: "修改联系人",
                        width: 900,
            ...
            $('#resultTable').find('a[data-action="delete"]').each(function (i, o) {
                $(o).on('click', function () {
                    ...
                    top.jBox.confirm('确定删除该联系人？', '', function (v, h, f) {
                        if (v == 'ok') {
                            $.post("deletesContactUser?checkcode="+checkCode, {guid: guid,jobNum:jobNum}, function (data) {
                                top.jBox.alert('删除成功');
                                inttRelation();
                            })
                        }
