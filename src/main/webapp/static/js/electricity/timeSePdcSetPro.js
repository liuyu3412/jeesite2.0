...
$(function () {
    ...
    var sleYear = $('#hidYear').val();
    if (sleYear) {
        var ops = $("#selectedYear option");
        for (var i = 0; i < ops.length; i++) {
            if (ops[i].text == sleYear) {
                ops[i].selected = true;
            }
        }
    }
...
function page(n, s) {
    ...
    if ($.trim($('#arrayList').val()) != '') {
        ...
        if (pageStart == false) {
            ...
            $('#searchForm').attr('action', $('#ctx').val() + '/electricity.balance/sePdcSetPro/listTime');
            $("#searchForm").submit();
        }
    }
}
...
$('table td').live('validate', function (evt, newValue) {
    if (evt.currentTarget.id != null && evt.currentTarget.id != '') {
        if (evt.currentTarget.attributes[0].nodeValue == "a") {
            if (evt.currentTarget.className == '1' || evt.currentTarget.className == '11') {
                ...
                if (booleanValue == false) {
                    ...
                    jBox.tip("输入数字长度不得大于6位！", "messager", {id: null, top: '15%'});
                    $(this).parent().attr('id', false);
                    $(this).attr('style', "box-shadow : inset 0 1px 1px rgba(0, 0, 0, 0.075), 0 0 6px red;outline: thin auto red;");
                } else if (eval(newValue) < eval(evt.currentTarget.id)) {
                    jBox.tip("输入数字必须 大于或等于 " + evt.currentTarget.id + "！", "messager", {id: null, top: '15%'});
            ...
            if (newValue.length > 6) {
                ...
                $(this).parent().attr('id', false);
                $(this).attr('style', "box-shadow : inset 0 1px 1px rgba(0, 0, 0, 0.075), 0 0 6px red;outline: thin auto red;");
            } else if (!(/^\d+(\.\d{1,2})?$/.test(booleanValue))) {
                jBox.tip("输入数字小数点最多保留2位！", "messager", {id: null, top: '15%'});
                $(this).parent().attr('id', false);
                $(this).attr('style', "box-shadow : inset 0 1px 1px rgba(0, 0, 0, 0.075), 0 0 6px red;outline: thin auto red;");
            } else if (eval(newValue) > eval(evt.currentTarget.id)) {
        ...
        if (evt.currentTarget.attributes[0].nodeValue == "energy") {
            var booleanValue = eval(newValue);
        ...
        } else if (evt.currentTarget.attributes[0].nodeValue == "price") {
            var booleanValue = eval(newValue);
            if (newValue.length > 6) {
                jBox.tip("输入数字长度不得大于6位！", "messager", {id: null, top: '15%'});
                $(this).parent().attr('id', false);
...
function old() {
    ...
    if ($("input[type='checkbox']:checked").length > 0) {
        ...
        // if($('#mktMonth').val() == time1 || $('#mktMonth').val() == ''){
        // 模拟等待完成操作
        jBox.tip("Loading...", 'loading');
        $('#searchForm').attr('action', $('#ctx').val() + '/electricity.balance/sePdcSetPro/oldTime');
        $('#searchForm').submit();
        // }else{
        // 	jBox.tip("请选择当前月份的数据！", "messager",{ id:null,top:'15%' });
        // }
    } else {
...
function excelSubmit() {
    var submit = function (v, h, f) {
        if (v == true) {
            ...
            $('#searchForm').attr('action', $('#ctx').val() + '/electricity.balance/sePdcSetPro/exportExcelTime');
            $('#searchForm').submit();
        }
        return true;
    };
...
function save() {
    ...
    if ($("input[class='checkboxList'][type='checkbox']:checked").length > 0) {
        ...
        if (booleanStart == false) {
            ...
            // 模拟等待完成操作
            jBox.tip("Loading...", 'loading');
            $('#searchForm').attr('action', $('#ctx').val() + '/electricity.balance/sePdcSetPro/saveTime')
            $('#searchForm').submit();
        }
    } else {
        ...
        if (booleanStart == false) {
            ...
            var submit = function (v, h, f) {
                if (v == true) {
                    // 模拟等待完成操作
                    jBox.tip("Loading...", 'loading');
                    $('#searchForm').attr('action', $('#ctx').val() + '/electricity.balance/sePdcSetPro/saveSubmitTime')
                    $('#searchForm').submit();
                }
                return true;
            };
